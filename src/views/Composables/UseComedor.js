...
export function useComedor() {
  ...
  const cargarPedidosAbiertos = async () => {
    const { data, error } = await supabase
      ...
      .eq("abierto", true)
      .eq("tipo", "Normal");
    if (error) { console.error("Error al obtener pedidos", error); return; }
  ...
  const cargarProductosPedido = async () => {
    ...
    // ✅ Guard para lista vacía — evita crash en .in() con array vacío
  ...
  // ── Calcular totales SIN guardar en BD (para visualización) ─
  ...
  // ── Calcular totales Y guardar en BD (legacy, por si se usa) ─
